...
int main(int argc, char* argv[])
{
    ...
    if(argc != 7)
    {
        cout << "usage: " << argv[0] << " Neighbor Orientation FrameNumber DensityNum DensityCoefficient PrintFrameIndex" << endl;
        return 0;
    }
...
*/
    ...
    {
        cout << Domains[i].cell.size() << endl;
        for(int j = 0; j < Domains[i].cell.size(); j++)
        {
            cout << Domains[i].cell[j] << endl;
        }
        //cout << endl;
    }
...
Domain FindDom(Domain Dom, int Index, vector<Cell> mon)
{
  ...
  {
    if((mon[Index].angle[i] <= 13)&&count(Dom.cell.begin(),Dom.cell.end(),mon[Index].neighbor[i])==0)
